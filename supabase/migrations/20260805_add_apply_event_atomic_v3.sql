CREATE OR REPLACE FUNCTION public.apply_event_atomic_v3(
  p_event_id text,
  p_user_id uuid,
...
BEGIN
  IF p_event_id IS NULL OR p_event_id = '' THEN
    RAISE EXCEPTION 'p_event_id cannot be empty';
  END IF;
...
REVOKE ALL ON FUNCTION public.apply_event_atomic_v3(text, uuid, text, text, smallint) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.apply_event_atomic_v3(text, uuid, text, text, smallint) FROM anon;
REVOKE ALL ON FUNCTION public.apply_event_atomic_v3(text, uuid, text, text, smallint) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.apply_event_atomic_v3(text, uuid, text, text, smallint) TO service_role;
