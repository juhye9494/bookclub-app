-- Migration: Create place_book_order_v2 RPC
-- DO NOT RUN YET. Run only after review.

CREATE OR REPLACE FUNCTION public.place_book_order_v2(
...
BEGIN
  ...
  IF p_shipping_name IS NULL OR length(trim(p_shipping_name)) < 1 OR length(trim(p_shipping_name)) > 100 THEN
    RAISE EXCEPTION '유효하지 않은 배송 수령인입니다.';
  END IF;
  
  IF p_shipping_phone IS NULL OR length(trim(p_shipping_phone)) < 1 OR length(trim(p_shipping_phone)) > 50 THEN
    RAISE EXCEPTION '유효하지 않은 배송 연락처입니다.';
  END IF;
  
  IF p_shipping_address IS NULL OR length(trim(p_shipping_address)) < 1 OR length(trim(p_shipping_address)) > 500 THEN
    RAISE EXCEPTION '유효하지 않은 배송 주소입니다.';
  END IF;
...
REVOKE ALL ON FUNCTION public.place_book_order_v2(uuid, uuid, uuid, text[], text, text, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.place_book_order_v2(uuid, uuid, uuid, text[], text, text, text) TO service_role;
