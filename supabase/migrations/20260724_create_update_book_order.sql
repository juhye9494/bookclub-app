-- Migration: Create update_book_order RPC
-- DO NOT RUN YET. Run only after review.

CREATE OR REPLACE FUNCTION public.update_book_order(
...
BEGIN
  -- 1. Lock the book_order
  SELECT * INTO v_book_order FROM public.book_orders 
  WHERE id = p_book_order_id
  FOR UPDATE;

  IF NOT FOUND THEN
    RAISE EXCEPTION '해당 도서 주문을 찾을 수 없습니다.';
  END IF;

  IF v_book_order.order_status != '주문접수' THEN
    RAISE EXCEPTION '배송 준비중일 경우 취소 및 변경이 불가능합니다.';
  END IF;

  -- 2. Lock the subscription order
  SELECT * INTO v_order FROM public.orders 
  WHERE id = v_book_order.subscription_order_id AND payment_status = 'DONE'
  FOR UPDATE;

  IF NOT FOUND THEN
    RAISE EXCEPTION '유효한 구독 결제 내역(DONE)을 찾을 수 없습니다.';
  END IF;

  IF v_order.cycle_id != v_book_order.cycle_id THEN
    RAISE EXCEPTION '주문 내역의 기수 정보가 일치하지 않습니다.';
  END IF;

  IF v_order.user_id != v_book_order.user_id THEN
    RAISE EXCEPTION '주문 내역의 사용자 정보가 일치하지 않습니다.';
  END IF;

  -- 3. Check Cycle bounds
  ...
  -- 4. Basic Validation for new books
  ...
  LOOP
    IF v_book_id IS NULL OR length(trim(v_book_id)) = 0 THEN
      RAISE EXCEPTION '유효하지 않은 도서 아이디가 포함되어 있습니다.';
    END IF;
  END LOOP;
  ...
  -- 5. Check max book count (excluding this specific book_order since we will replace its items)
  ...
  LOOP
    ...
    IF FOUND THEN
      RAISE EXCEPTION '이미 다른 주문에서 신청한 도서가 포함되어 있습니다: %', v_trimmed_book_id;
    END IF;
  END LOOP;
  ...
  -- Touch updated_at on book_orders
  UPDATE public.book_orders SET updated_at = NOW() WHERE id = p_book_order_id;

  RETURN jsonb_build_object('success', true);
END;
$$;

REVOKE ALL ON FUNCTION public.update_book_order(uuid, text[]) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.update_book_order(uuid, text[]) TO service_role;
