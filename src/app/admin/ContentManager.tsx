...
export default function ContentManager() {
  ...
  const loadData = async () => {
    ...
    const { data: booksData } = await supabase.from('books').select('*').order('order_idx', { ascending: true });
    if (booksData) {
      setBooks(booksData);
    }
    setLoading(false);
  ...
  const activeCycleBooks = books.filter(b => b.cycle_id === activeCycle?.id && !b.is_deleted);
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {activeCycleBooks.map((book: any, i: number) => (
                  <div key={book.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fff', padding: '16px', borderRadius: '8px', border: '1px solid #e5dfd2', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
                    ...
                    <div>
                      <button onClick={() => { setEditingBook(book); setIsCreatingBook(false); }} style={{ padding: '6px 12px', background: '#fff', border: '1px solid #cfc8b8', borderRadius: '4px', cursor: 'pointer', fontSize: '0.8rem' }}>편집</button>
                    </div>
                  </div>
                ))}
